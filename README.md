...
```text
HOMEDASH_IMAGE=ghcr.io/immer-gut/homedash:v1.8.0
HOMEDASH_PORT=3002
...
- Wenn das Image auf `latest` steht, in Portainer vor dem Test ein Pull/Redeploy ausfuehren. Fuer stabile Deployments ist ein Versions-Tag wie `v1.8.0` besser, ja, erstaunlicherweise hilft Versionierung beim Versionieren.
...
services:
  homedash:
    image: ghcr.io/immer-gut/homedash:v1.8.0
...
| --- | --- | --- |
| `HOMEDASH_IMAGE` | `ghcr.io/immer-gut/homedash:v1.8.0` | Docker Image fuer den Stack. Fuer einfache Tests kann `latest` genutzt werden, fuer Portainer besser einen Versions-Tag setzen. |
...
```text
ghcr.io/immer-gut/homedash:v1.8.0
```
...
- Datums-Widget fuer Tage bis zu einem Datum oder Tage seit einem Datum
- Mehrere Notizen
...
Das Datums-Widget nutzt einen Titel und ein Datum im Format `YYYY-MM-DD`. Liegt das Datum in der Zukunft, zeigt Homedash die Tage bis dahin. Liegt es in der Vergangenheit, zeigt Homedash die Tage seitdem. Liegt es auf heute, steht da ausnahmsweise mal etwas Beruhigendes: `Heute`.
...
1. Stack oeffnen.
2. Bei gepinnten Versionen `HOMEDASH_IMAGE` auf den neuen Tag setzen, zum Beispiel `ghcr.io/immer-gut/homedash:v1.8.0`.
