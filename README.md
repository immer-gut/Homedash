...
```text
HOMEDASH_IMAGE=ghcr.io/immer-gut/homedash:v1.8.3
HOMEDASH_PORT=3002
...
- Wenn das Image auf `latest` steht, in Portainer vor dem Test ein Pull/Redeploy ausfuehren. Fuer stabile Deployments ist ein Versions-Tag wie `v1.8.3` besser, ja, erstaunlicherweise hilft Versionierung beim Versionieren.
...
services:
  homedash:
    image: ghcr.io/immer-gut/homedash:v1.8.3
...
| --- | --- | --- |
| `HOMEDASH_IMAGE` | `ghcr.io/immer-gut/homedash:v1.8.3` | Docker Image fuer den Stack. Fuer einfache Tests kann `latest` genutzt werden, fuer Portainer besser einen Versions-Tag setzen. |
...
```text
ghcr.io/immer-gut/homedash:v1.8.3
```
...
1. Stack oeffnen.
2. Bei gepinnten Versionen `HOMEDASH_IMAGE` auf den neuen Tag setzen, zum Beispiel `ghcr.io/immer-gut/homedash:v1.8.3`.
